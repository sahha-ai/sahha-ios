...
extension SahhaSensor {
    ...
    private static let reverseLookup: [String: SahhaSensor] = {
        var lookup: [String: SahhaSensor] = [:]
        for sensor in Self.allCases {
            if let objectType = sensor.hkObjectType {
                lookup[objectType.identifier] = sensor
            }
        }
        return lookup
    }()
    ...
    static func sensor(for objectType: HKObjectType) -> SahhaSensor? {
        return reverseLookup[objectType.identifier]
    }
}
