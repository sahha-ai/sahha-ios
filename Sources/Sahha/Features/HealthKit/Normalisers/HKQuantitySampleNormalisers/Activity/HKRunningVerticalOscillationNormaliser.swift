...
struct HKRunningVerticalOscillationNormaliser: HKNormaliser {
    func normalise(sample: HKSample) -> [DataLog]? {
        guard #available(iOS 16.0, *) else { return nil }
        
        let type = HKQuantityType.quantityType(forIdentifier: .runningVerticalOscillation)
        
        guard let quantitySample = sample as? HKQuantitySample, quantitySample.quantityType == type else {
            return nil
        }
        
        let value = quantitySample.quantity.doubleValue(for: .meterUnit(with: .centi)).rounded(toPlaces: 4)
